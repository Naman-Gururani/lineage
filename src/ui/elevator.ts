...
export function openElevator(): void {
        ...
        <div class="elev-buttons" role="group" aria-label="Floors">
          ${FLOORS.map(
            (f, i) =>
              ...
              `<b aria-hidden="true">${f.key}</b><span>${esc(f.label)}${f.sub ? `<small>${esc(f.sub)}</small>` : ''}</span></button>`,
          ).join('')}
        </div>
      </div>
      <div class="elev-car">
  ...
  const cardHTML = (i: number): string => {
    if (i === 0)
      return `<p class="elev-kicker">GROUND FLOOR</p><h3>Welcome to Barclays Tower</h3><p>Naman's floors of the bank, one ride at a time. Pick a floor — the lift is slow, the streams are not.</p>`
