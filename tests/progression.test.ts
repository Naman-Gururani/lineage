...
describe('QuestLog', () => {
  it('starts, advances with clamping, and completes once', () => {
    ...
    expect(log.isActive('balloons')).toBe(false)
    log.start('balloons')
    expect(log.isActive('balloons')).toBe(true)
    log.advance('balloons', 'find', 3)
    expect(log.progress('balloons')).toEqual({ done: 3, total: 5 })
    log.advance('balloons', 'find', 10)
    expect(log.progress('balloons')).toEqual({ done: 5, total: 5 })
    expect(log.isDone('balloons')).toBe(false)
    log.advance('balloons', 'return', 1)
    expect(log.isDone('balloons')).toBe(true)
    log.advance('balloons', 'return', 1)
    expect(fired.filter((f) => f === 'done:balloons').length).toBe(1)
    expect(fired[0]).toBe('started:balloons')
  })
  ...
  it('ignores unknown quests and persists state', () => {
    ...
    log.start('tickets')
    log.advance('tickets', 'collect', 4)
    const again = new QuestLog(log.state, () => {})
    expect(again.progress('tickets').done).toBe(4)
  })

  // The full table — ids, givers, targets — is pinned in `quests.test.ts`.
  it('keeps the story and the fair errands, each with a reward', () => {
    const ids = new Set(QUESTS.map((q) => q.id))
    for (const id of ['balloons', 'crew', 'ducks', 'explore', 'lights', 'story', 'tickets']) expect(ids.has(id)).toBe(true)
